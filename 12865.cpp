...
int main() 
{
    ...
    vvi dp(n+1, vi(k+1, 0)); // dp[i][j] is the maximum value you can carry using first i objects and j weight
    for (int i=0; i<n; i++)
    {
    	for (int j=0; j<=k; j++)
    	{
    		...
    		if (weights[i]+j <= k)
    			dp[i+1][weights[i]+j] = max(dp[i+1][weights[i]+j], dp[i][j] + values[i]);

    		// don't take the ith element
