...
void backtrack(vi number, vi symbols)
{
	if (number.size() == 1)
	{
		large = max(number[0], large);
		small = min(number[0], small);
		return;
	}

	int a = number.back(); number.pop_back();
	int b = number.back(); 
...
int main() 
{
    ...
    for (int i=0; i<4; i++) cin >> symbols[i];
