...
ll solve(int n, vi& coins)
{
	...
	ll ans = INT_MAX;
	for (auto coin : coins)
		ans = min(ans, 1 + solve(n-coin, coins));
...
int main() 
{
    ...
    dp = vl(k+1, INT_MAX); // dp[i] is the minimum coins to make i
    // dp = vl(k+1, -1); // dp[i] is the minimum coins to make i

    dp[0] = 0;
    for (int i=0; i<=k; i++)
    	for (int j=0; j<n; j++) if (coins[j] + i <= k)
    		dp[coins[j]+i] = min(dp[i]+1, dp[i+coins[j]]);

    cout << (dp[k] == INT_MAX ? -1 : dp[k]) << '\n';

    // cout << (solve(k, coins) == INT_MAX ? -1 : solve(k, coins)) << ' ';
