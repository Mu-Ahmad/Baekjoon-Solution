...
vector <vector <set<pii>> > adj_list;
vvi colors;

bool available(int c, int i, int j)
{
	for (auto node : adj_list[i][j])
	{
		if (colors[node.fi][node.se] == c) return false;
	}
...
int main() 
{
    ...
    adj_list = vector <vector <set<pii>> > (n, vector <set<pii>>(n));
    ...
    for (int i=0; i<n; i++)
    	for (int j=0; j<n; j++)
    	{
    		for (int k=0; k<6; k++)
    		{
    			...
    			if (valid(_i, _j) and grid[_i][_j] == 'X')
    			{
    				adj_list[_i][_j].insert({i, j});
    				adj_list[i][j].insert({_i, _j});
    			}
    		}
    	}
	...
	for (int i=0; i<n; i++)
		for (int j=0; j<n; j++) if (grid[i][j] == 'X' and colors[i][j] == 0)
		{	
			int c = 1;
			while (!available(c, i, j)) c++;
