...
int calculateScore(vi a, vi b)
{
	int score1 = 0;
	REP(i, a.size())
		REP(j, a.size()) score1 += grid[a[i]][a[j]];

	int score2 = 0;
	REP(i, b.size())
		REP(j, b.size()) score2 += grid[b[i]][b[j]];

	return abs(score1-score2);
}

void solve(int curr, vi team1, vi team2)
{
	if (curr == n)
	{
		if (team1.size() and team2.size())
		{
			ans = min(ans, calculateScore(team1, team2));
		}
