...
ll solve(int curr, ll cost, int mask)
{
	if (__builtin_popcountll(mask) == n)
	{
		cost += grid[curr][start];
		return cost;
	}
	

	ll ans = INT_MAX;
	for (int i=0; i<n; i++) if (i != curr and grid[curr][i] != INT_MAX and !(mask&(1<<i)))
		ans = min(ans, solve(i, cost+grid[curr][i], mask|(1<<i)));
