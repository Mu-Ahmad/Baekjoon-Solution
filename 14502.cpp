...
bool valid(pii p)
{
	if (p.fi<0 or p.fi>=n or p.se<0 or p.se>=m) return false;
    return true;
}

void countSafePlaces(vvi copyGrid)
{
	int nax = 0;
	for (int i=0; i<n; i++)
    	for (int j=0; j<m; j++) if (copyGrid[i][j] == 0)
    		nax++;
...
{
	auto copyGrid = grid;
	vvi visted(n, vi(m, false));
	queue<pii> q;

    for (int i=0; i<n; i++)
    	for (int j=0; j<m; j++) if (copyGrid[i][j] == 2)
    	{
    		visted[i][j] = true;
    		q.push({i, j});
    	}
    ...
    while(q.size())
    {
    	...
    	for (int i=0; i<4; i++)
    	{
    		pii nxt = {curr.fi+dx[i], curr.se+dy[i]};

    		if (valid(nxt) and !visted[nxt.fi][nxt.se] and copyGrid[nxt.fi][nxt.se] == 0)
    		{
    			visted[nxt.fi][nxt.se] = true;
    			copyGrid[nxt.fi][nxt.se] = 2;
    			q.push(nxt);
    		}
    	}
    }

	countSafePlaces(copyGrid);
}
