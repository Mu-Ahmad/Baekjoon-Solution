...
int main() 
{
    ...
    // state: dp[i][c] is the optimal answer for painting i houses with last being of color c
    // ----- 

    dp[0][0] = dp[0][1] = dp[0][2] = 0;

    //push dp
    // transitions: no two houses will have same colors 
    // ----------- 
    for (int i=0; i<n; i++)
    {
    	dp[i+1][0] = min({dp[i+1][0], dp[i][1] + cost[i][0], dp[i][2] + cost[i][0]});

    	dp[i+1][1] = min({dp[i+1][1], dp[i][0] + cost[i][1], dp[i][2] + cost[i][1]});
    
    	dp[i+1][2] = min({dp[i+1][2], dp[i][0] + cost[i][2], dp[i][1] + cost[i][2]});
    }
