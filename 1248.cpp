...
bool valid(int curr)
{
	...
	{
		sum += seq[i];
		if (sum == 0 and symbol_table[i][curr] != 0) return false;
		if (sum > 0 and symbol_table[i][curr] != 1) return false;
		if (sum < 0 and symbol_table[i][curr] != -1) return false;
	}
...
void dfs(int curr)
{
	...
	for (int i=1; i<11; i++)
	{
		seq[curr] = i * symbol_table[curr][curr];
		if (valid(curr))
			dfs(curr+1);
	}
}
...
int main() 
{
    ...
    for (int i=0; i<n; i++)
    	...
    	{
    		cin >> ch;
    		if (ch == '-') symbol_table[i][j] = -1;
    		else if (ch == '+') symbol_table[i][j] = 1;
    		else symbol_table[i][j] = 0;
    	}
